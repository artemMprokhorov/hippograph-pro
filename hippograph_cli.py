#!/usr/bin/env python3
"""
hippograph-cli: Agent-native CLI for HippoGraph Pro memory system.

Usage:
  hippograph search <query> [--limit N] [--json] [--category CAT]
  hippograph add <content> [--category CAT] [--intensity N] [--importance critical|normal|low]
  hippograph stats
  hippograph status
  hippograph sleep
  hippograph pcb
...
Env vars:
  HIPPOGRAPH_API_URL   Base URL (default: http://localhost:5001)
  HIPPOGRAPH_API_KEY   API key
"""
        ...
        for k, v in stats.items():
            if isinstance(v, dict):
                ...
                for kk, vv in v.items():
                    print(f'    {kk}: {c("bold", str(vv))}')
            else:
                print(f'  {c("cyan", k)}: {c("bold", str(v))}')
    print()
...
def cmd_status(args):
    ...
    try:
        ...
        print(c('yellow', f'Docker not available: {e}'))
...
def cmd_sleep(args):
    ...
    if not args.no_wait:
        print(c('gray', '   Monitoring... (Ctrl+C to stop)'))
        import time
        try:
            while True:
                time.sleep(10)
...
def cmd_pcb(args):
    """Quick memory continuity check."""
    questions = [
        ('What is spreading activation?', ['activation', 'graph', 'search', 'spreading']),
        ('What is BGE-M3?', ['embedding', 'BGE', 'bge', 'model', 'retrieval']),
        ('What is LOCOMO benchmark?', ['benchmark', 'recall', 'retrieval', 'LOCOMO']),
        ('What is late chunking?', ['chunk', 'overlap', 'parent', 'session']),
        ('What is sleep compute?', ['sleep', 'consolidation', 'PageRank', 'community']),
    ]
    print(c('bold', '\n🧪 Quick Memory Check'))
    passed = 0
    for q, keywords in questions:
        ...
        print(f'  {icon} {q}')
        if not ok:
            print(c('gray', f'     Expected one of: {keywords}'))
        else:
            passed += 1
...
def cmd_repl(args):
    ...
    while True:
        try:
            line = input(c('cyan', 'hippograph> ')).strip()
        ...
        try:
            if cmd == 'search':
                ...
                print(c('gray', '  search | add | stats | sleep | graph | get | pcb | status | exit'))
            else:
                print(c('yellow', f'  Unknown: {cmd}. Type help.'))
        except Exception as e:
            print(c('red', f'  Error: {e}'))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='hippograph',
        description='Agent-native CLI for HippoGraph Pro memory system',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  hippograph search "temporal retrieval results"
  hippograph search "spreading activation" --json --limit 3
  hippograph add "Deployed new config" --category milestone --intensity 7
  hippograph stats
  hippograph sleep
  hippograph pcb
  hippograph graph 42
...
'''
    ...
    p.add_argument('--intensity', type=int, default=5)
    p.add_argument('--tone', '-t')
    p.add_argument('--json', action='store_true')
